...
impl<'a> Stage0<'a> {
    ...
    async fn request_app_keys_from_kms_url(&self, kms_url: String) -> Result<AppKeys> {
        ...
        let response = kms_client
            .get_app_key(rpc::GetAppKeyRequest {
                ...
                key_version: 0, // Use active version by default
            })
            .await
    ...
    async fn request_app_keys(&self) -> Result<AppKeys> {
        let key_provider = self.shared.app_compose.key_provider(); // 获取密钥提供者类型
        match key_provider {
            KeyProviderKind::Kms => self.request_app_keys_from_kms().await, // 从KMS获取密钥
            KeyProviderKind::Local => self.get_keys_from_local_key_provider().await, // 从本地密钥提供者获取密钥
            KeyProviderKind::None => {
                info!("No key provider is enabled, generating temporary app keys"); // 没有密钥提供者，生成临时密钥
                let seed: [u8; 32] = rand::thread_rng().gen();
                gen_app_keys_from_seed(&seed, None).context("Failed to generate app keys") // 生成临时密钥
            }
        }
    }

    async fn mount_data_disk(&self, initialized: bool, disk_crypt_key: &str) -> Result<()> {
        let name = "dstack_data_disk"; // LUKS设备名称
        let fs_dev = "/dev/mapper/".to_string() + name; // 设备路径
        let mount_point = &self.args.mount_point; // 挂载点
        if !initialized { // 如果数据盘未初始化
            self.vmm
                .notify_q("boot.progress", "initializing data disk") // 通知主机正在初始化数据盘
                .await;
            info!("Setting up disk encryption"); // 设置磁盘加密
            self.luks_setup(disk_crypt_key, name)?;
            cmd! {
                mkdir -p $mount_point; // 创建挂载点
                zpool create -o autoexpand=on dstack $fs_dev; // 创建zpool，启用自动扩展
                zfs create -o mountpoint=$mount_point -o atime=off -o checksum=blake3 dstack/data; // 创建zfs文件系统
            }
            .context("Failed to create zpool")?;
        } else {
            self.vmm
                .notify_q("boot.progress", "mounting data disk") // 通知主机正在挂载数据盘
                .await;
            info!("Mounting encrypted data disk");
            let root_hd = &self.args.device;
            let disk_crypt_key = disk_crypt_key.trim();
            cmd!(echo -n $disk_crypt_key | cryptsetup luksOpen --type luks2 -d- $root_hd $name) // 打开加密数据盘
                .or(Err(anyhow!("Failed to open encrypted data disk")))?;
            cmd! {
                zpool import dstack; // 导入zpool
                zpool status dstack; // 查看zpool状态
                zpool online -e dstack $fs_dev; // 在线扩展zpool
            }
            .context("Failed to import zpool")?;
            if cmd!(mountpoint -q $mount_point).is_err() {
                cmd!(zfs mount dstack/data).context("Failed to mount zpool")?; // 挂载zfs文件系统
            }
        }
        Ok(())
    }
