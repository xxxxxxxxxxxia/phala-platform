// src/components/IncentiveFlow.tsx
...
import WorkerDashboard from './WorkerDashboard'; // 修改: 导入新的拆分组件
import PoolDashboard from './PoolDashboard';   // 修改: 导入新的拆分组件
import AssetDashboard from './AssetDashboard';   // 修改: 导入新的拆分组件
import SessionManager from './SessionManager';
import TokenomicEditor from './TokenomicEditor';
import StopComputing from './StopComputing';
import Console from './Console';
import Modal from './Modal';
import Toast from './Toast';
import SystemOperations from './SystemOperations'; // 新增: 导入新的系统操作页面组件
...
const IncentiveFlow = (): React.ReactElement => {
    ...
    const [stopComputingSender, setStopComputingSender] = useState<string>('');
    const [stopComputingPid, setStopComputingPid] = useState<string>('0');
    const [stopComputingWorker, setStopComputingWorker] = useState<string>(DEFAULT_GATEKEEPER_PUBKEY);
    ...
    const loadAccounts = useCallback(async (): Promise<void> => {
        ...
        try {
            ...
            if (devAccounts.length > 0) {
                ...
                setStopComputingSender(aliceAddress);
    ...
    const stopComputing = useCallback(() => {
        if (!api || !stopComputingSender) return handleError("请选择操作账户。");
        const extrinsic = api.tx.phalaStakePoolv2.stopComputing(stopComputingPid, stopComputingWorker);
        sendTransaction(extrinsic, false, stopComputingSender);
    }, [api, sendTransaction, stopComputingSender, stopComputingPid, stopComputingWorker, handleError]);
